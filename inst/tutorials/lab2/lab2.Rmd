...
## Exponential Distribution Parameterization

How is the Exponential distribution parameterized in R? How have we/the book parameterized it?
...
So far we have made histograms to examine a single distribution. We use a scatterplot to investigate the relationship between two quantitative variables. We may also want to quantify the correlation between the two variables using the `cor` function.
...
You should have a template.Rmd in your project. Open it, copy and paste it into a new Rmd file and proceed. Open it, copy and paste it into a new Rmd file and proceed (in the top left corner, there is a button with a white square and a green plus sign icon, then choose the RMarkdown option).
...
*Due*: Wednesday, October 13th at 5 PM
...
**(i)** Calculate the median of this distribution using the `qexp` function. Pay attention to the parameterization of the Exponential distribution in R.
