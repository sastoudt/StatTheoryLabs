...
If you have completed HW 8, you can skip to the "What does capture-recapture data tell us about the detection probability?" section.

If you have not completed HW 8 (or want a refresher), read through the next six sections before continuing.
...
If we assume we never mis-identify a species i.e. never have a false positive (which is a pretty big assumption), we are still left with th epossibility that we didn't see an individual of the species, even when it was there (false negative). Therefore, our $\hat{N}$ is a lower bound on $N$.
...
**Question: For three scenarios, use the hover feature to approximate the maximum of the likelihood curve. Write down the paramters you used and the values of the maximum points. Compare those values to the two value of the two estimators we have seen so far (LP and MoM).**
...
**Question: When does this get close to the truth? Try a few combinations and note what you observe.**

**Question: How can we be really wrong? So far we've been worried about underestimating. Try to find a situation where we overestimate the abundance. How likely is that situation to occur? Hint: consider the implied detection probability.**
...
The estimate approaches the truth (is asymptotically unbiased) as the "sample size" gets larger. I personally find it a bit ambiguous which sample size we are referring to: $n_1$? $n_2$? $N$? $m_2$?
...
In the extreme case, if $n_1 = N$, then we need $m_2 = n_1$ to get our estimate exactly right. If $n_1 < N$ then the closest we can get to the truth is $\frac{n_1 n_2}{n_1} = n_2$ when $m_2 = n_1$. 
...
**Question: What situations minimize the variance? Try a few combinations and note what you observe.**

```{r,  echo=FALSE}
sliderInput("N4", "true abundance", min = 10, max = 100, value = 10,step = 5)
sliderInput("n_14", "number seen and tagged on day 1 (should be less than N)", min = 0, max = 100, value = 5,step = 5)
...
output$varPlot <- renderPlotly({
  ...
  var <- input$n_24 *(input$n_14/input$N4)*((input$N4-input$n_14)/input$N4)*((input$N4 - input$n_24)/(input$N4 -1))
...
```{r,  echo=FALSE}
sliderInput("N5", "true abundance", min = 10, max = 100, value = 20,step = 5)
sliderInput("n_15", "number seen and tagged on day 1 (should be less than N)", min = 0, max = 100, value = 10,step = 5)
sliderInput("m_25", "number recaptured on day 2 (should be less than n_1)", min = 0, max = 100, value = 5,step = 5)

plotlyOutput("varPlot2")
```

```{r,  context="server"}
output$varPlot2 <- renderPlotly({
  n_2 <- seq(0, input$N5, by = 1)
  var <- n_2 *(input$n_15/input$N5)*((input$N5-input$n_15)/input$N5)*((input$N5 - input$n_25)/(input$N5 -1))
  toPlot <- cbind.data.frame(n_2 = n_2, var=var)

 g <- ggplot(toPlot, aes(n_2, var))+geom_point()+theme_minimal(base_size = 20)+xlab("number seen on day 2")+ylab("variance")
 ggplotly(g)
...
Chapman adjusted the Lincoln-Peterson estimate such that it is an exactly unbiased estimator of $N$ when $n_1 + n_2 \geq N$.

$$\hat{N}_C = \frac{(n_1+1)(n_2+1)}{(m_2 + 1)} - 1$$

**some slider here + question**
...
- Each sample of teh population is a random sample.
